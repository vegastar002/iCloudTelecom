...
public class ChatroomMemberManagerActivity extends GroupBaseActivity {
	...
	private void initialize(Bundle savedInstanceState) {
		...
		VoiceHelper.getInstance().getDevice().queryMembersWithChatroom(roomNo);
	}
				...
				holder.kickButton.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if(mKickAdapter != null ) {
							showConnectionProgress(getString(R.string.str_dialog_message_default));
							VoiceHelper.getInstance().getDevice().reomoveMemberFromChatroom(CCPConfig.App_ID, roomNo, chatroomMember.getNumber());
	...
	private android.os.Handler mChatRoomHandler = new android.os.Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case VoiceHelper.WHAT_ON_CHATROOM_KICKMEMBER:
				try {
					closeConnectionProgress();
					if(reason != 0) {
						BaseApplication.getInstance().showToast(getString(R.string.str_member_kicked_out_failed, reason + ""));
						return;
					}
